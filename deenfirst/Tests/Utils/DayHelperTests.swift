...
final class DayHelperTests: XCTestCase {
    ...
    func testGetCurrentDayName_ReturnsValidDayName() {
        ...
        XCTAssertTrue(
            DayHelper.allDayNames.contains(dayName), "Day name should be one of the valid day names"
        )
    }
    ...
    func testGetCurrentDayIndex_MatchesCurrentDayName() {
        ...
        XCTAssertEqual(
            DayHelper.allDayNames[index], dayName, "Index should match the current day name")
    }
}
