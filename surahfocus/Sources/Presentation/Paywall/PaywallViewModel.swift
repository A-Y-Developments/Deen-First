...
final class PaywallViewModel: ObservableObject {
    ...
    private func checkTrialEligibility() async {
        ...
        if let monthly = monthlyPackage { productIdentifiers.append(monthly.storeProduct.productIdentifier) }
        if let yearly = yearlyPackage { productIdentifiers.append(yearly.storeProduct.productIdentifier) }
        ...
        if let monthly = monthlyPackage {
            monthlyTrialEligible = eligibility[monthly.storeProduct.productIdentifier]?.status == .eligible
        }
        if let yearly = yearlyPackage {
            yearlyTrialEligible = eligibility[yearly.storeProduct.productIdentifier]?.status == .eligible
        }
    }
    ...
    func purchase() async -> Bool {
        ...
        do {
            guard let package = selectedPlan == .monthly ? monthlyPackage : yearlyPackage else {
                throw SubscriptionError.packageNotFound(selectedPlan == .monthly ? "monthly" : "yearly")
            }
            let success = try await DIContainer.shared.subscriptionService.purchase(package: package)
            if success {
