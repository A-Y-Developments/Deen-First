...
final class LocalDataSource {
    ...
    // MARK: - AppLimit Operations

    @MainActor
    func insertAppLimit(_ limit: AppLimit) throws {
        context.insert(limit)
        try context.save()
    }

    @MainActor
    func getAllAppLimits() throws -> [AppLimit] {
        let descriptor = FetchDescriptor<AppLimit>(
    ...
    @MainActor
    func getAppLimit(id: UUID) throws -> AppLimit? {
        let descriptor = FetchDescriptor<AppLimit>(
    ...
    @MainActor
    func updateAppLimit(_ limit: AppLimit) throws {
        try context.save()
    }

    @MainActor
    func deleteAppLimit(_ limit: AppLimit) throws {
        context.delete(limit)
        try context.save()
    }

    // MARK: - TimeLimit Operations

    @MainActor
    func insertTimeLimit(_ limit: TimeLimitSettings) throws {
        context.insert(limit)
        try context.save()
    }

    @MainActor
    func getAllTimeLimits(for userId: UUID) throws -> [TimeLimitSettings] {
        let descriptor = FetchDescriptor<TimeLimitSettings>(
    ...
    @MainActor
    func getTimeLimit(id: UUID) throws -> TimeLimitSettings? {
        let descriptor = FetchDescriptor<TimeLimitSettings>(
            predicate: #Predicate { $0.id == id }
        )
        let limits = try context.fetch(descriptor)
    ...
    @MainActor
    func updateTimeLimit(_ limit: TimeLimitSettings) throws {
        try context.save()
    }

    @MainActor
    func deleteTimeLimit(_ limit: TimeLimitSettings) throws {
        context.delete(limit)
        try context.save()
    }
}
